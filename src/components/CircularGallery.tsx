...
// Utility functions (typed loosely for brevity)
...
class Media {
  ...
  createShader() {
    ...
    (texture as any).minFilter = (this.gl as any).LINEAR_MIPMAP_LINEAR;
    (texture as any).magFilter = (this.gl as any).LINEAR;
    ...
    this.program = new Program(this.gl as any, {
      ...
      uniforms: {
        ...
        uBorderRadius: { value: (this as any).borderRadius },
      },
      transparent: true,
    });
    const img = new Image();
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    // Increased size by ~100% for 3-4 images to show simultaneously
...
class App {
  ...
  constructor(
    ...
    this.autoScrollSpeed = autoScroll ? autoScrollSpeed : 0;
  ...
  createRenderer() {
    ...
    this.renderer.dpr = Math.min(window.devicePixelRatio || 1, 2.5);
  ...
  createMedias(items: any[], bend = 1, borderRadius: number, textColor = "#ffffff", font = "bold 30px sans-serif") {
    const defaultItems: any[] = [];
    const galleryItems = items && items.length ? items : defaultItems;
    this.galleryLength = galleryItems.length;
    // Triple the items so content exists on both left and right initially
  ...
  onTouchMove(e: any) {
    ...
    const distance = (this.start - x) * (this.scrollSpeed * 0.045);
  ...
  onCheck() {
    ...
    const width = (this.medias[0] as any).width;
  ...
  onResize() {
    ...
    if (this.medias) {
      this.medias.forEach((media) => (media as any).onResize({ screen: this.screen, viewport: this.viewport }));
    }
  }
  update() {
    // Auto-scroll to the left by increasing target slightly each frame
    this.scroll.target += this.autoScrollSpeed;
    this.scroll.current = lerp(this.scroll.current, this.scroll.target, this.scroll.ease);
    const direction = this.scroll.current > this.scroll.last ? "right" : "left";
    if (this.medias) {
      this.medias.forEach((media) => (media as any).update(this.scroll, direction));
    }
  ...
  addEventListeners() {
    ...
    // Add wheel listeners to enable scroll-to-drag functionality
    const target = (this.gl && this.gl.canvas) ? this.gl.canvas : this.container;
    ...
    target.addEventListener("touchstart", this.boundOnTouchDown);
...
export default function CircularGallery({
  ...
  return (
    <div className="circular-gallery" ref={containerRef} style={{ position: 'relative' }} />
  );
}